...
if exists('current_compiler')
  ...
  let current_compiler = 'iced'
endif

" Pattern to check if iced is the compiler
let s:pat = '^' . current_compiler

" Path to IcedCoffeeScript compiler
...
function! s:GetMakePrg()
  return g:coffee_compiler.' -c ' . g:coffee_make_options . ' $* ' . fnameescape(expand('%'))
endfunction

" Set `makeprg` and return 1 if iced is still the compiler, else return 0.
...
augroup CoffeeUpdateMakePrg
  ...
  " Update `makeprg` if iced is still the compiler, else stop running this
  " function.
