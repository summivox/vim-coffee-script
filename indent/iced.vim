...
setlocal indentexpr=GetIcedIndent(v:lnum)
" Make sure GetIcedIndent is run when these are typed so they can be
" indented or outdented.
setlocal indentkeys+=0],0),0.,=else,=when,=catch,=finally

" Only define the function once.
if exists("*GetIcedIndent")
  finish
endif
...
let s:SYNTAX_STRING = 'iced\%(String\|AssignString\|Embed\|Regex\|Heregex\|'
\                   . 'Heredoc\)'

" Syntax names for comments
let s:SYNTAX_COMMENT = 'iced\%(Comment\|BlockComment\|HeregexComment\)'
...
function! GetIcedIndent(curlinenum)
